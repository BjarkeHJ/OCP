...
void RosaPoints::rosa_main(pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud) {
    ...
    /* Visualization */
    vis_curr_cloud = RC.pts_;
    vis_rosa_pts = skeleton_ver_cloud;
    std::cout << skeleton_ver_cloud->points.size() << std::endl;

    // pcl::PointCloud<pcl::PointXYZ>::Ptr temp_cloud(new pcl::PointCloud<pcl::PointXYZ>);
    // for (int i=0; i<pset.rows(); i++) {
    //     pcl::PointXYZ point_temp;
    //     point_temp.x = pset(i,0);
    //     point_temp.y = pset(i,1);
    //     point_temp.z = pset(i,2);
    //     temp_cloud->push_back(point_temp);
    // }
    // vis_rosa_pts = temp_cloud;
}
